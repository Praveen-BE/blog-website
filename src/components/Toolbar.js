...
const Toolbar = () => {
  ...
  return (
    <div>
      <button
        className={`font-bold mx-1 size-8 rounded-md ${isBold ? "bg-gray-200" : " "}`}
        onClick={() => {
      ...
      <button
        className={`italic mx-1 size-8 rounded-md ${isItalic ? "bg-grey-200" : " "}`}
        onClick={() => {
      ...
      <ImageUpload onUpload={(url) => insertImage(url)} />
    </div>
  );
};
